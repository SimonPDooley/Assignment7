...
public class CustomArrayList<T> implements CustomList<T> {
	...
	public boolean add(T item) {
		...
		if (items[arraySize - 1] == null) {
			...
			Object[] arrayDoubledInSize = new Object[arraySize * 2];

			if (items[items.length - 1] != null) {
				arraySize = arrayDoubledInSize.length;
			}
			// Copy elements of old array to new array
			for (int i = 0; i < items.length; i++) {
				arrayDoubledInSize[i] = items[i];
			}
			items = arrayDoubledInSize;
			arrayDoubledInSize[itemsPosition] = item;
			itemsPosition++;
		}
		return true;
	}
